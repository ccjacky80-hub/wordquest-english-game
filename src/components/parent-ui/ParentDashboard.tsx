...
export function ParentDashboard() {
  ...
  const refresh = async () => {
    const [attempts, progress, localEvents] = await Promise.all([repository.getAllAttempts(), repository.getAllWordProgress(), repository.getAllLocalEvents()]);
    setSnapshot(buildParentDashboardSnapshot(attempts, progress, localEvents));
  };
  ...
  useEffect(() => { let cancelled = false; const repo = createProgressRepository(); Promise.all([repo.getAllAttempts(), repo.getAllWordProgress(), repo.getAllLocalEvents()]).then(([attempts, progress, localEvents]) => { if (!cancelled) setSnapshot(buildParentDashboardSnapshot(attempts, progress, localEvents)); }); return () => { cancelled = true; }; }, []);
  ...
  return <main className="parent-shell">
    ...
    <section className="parent-panel telemetry-panel"><div className="parent-panel-heading"><div><p className="parent-kicker">LOCAL TELEMETRY</p><h2>7-day event record</h2></div><span className="parent-table-note">Stored on this device only</span></div><div className="telemetry-summary">{Object.entries(snapshot.localEventSummary).filter(([, count]) => count > 0).map(([type, count]) => <div key={type}><strong>{type.replaceAll('_', ' ')}</strong><b>{count}</b></div>)}{Object.values(snapshot.localEventSummary).every((count) => count === 0) ? <p className="parent-muted">No local events recorded yet.</p> : null}</div></section>
