...
export function buildParentDashboardSnapshot(
  ...
  return { dailyProgress: computeDailyProgress(attempts), overallAccuracy: totalAttempts ? Math.round((totalCorrect / totalAttempts) * 100) : 0, totalAttempts, totalCorrect, gamePerformance, words, recentActivity, confusionPairs: aggregateConfusionPairs(attempts) };
}
